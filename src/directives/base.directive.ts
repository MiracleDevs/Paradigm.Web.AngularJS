...
export abstract class DirectiveBase implements IDirective {
    ...
    protected call<T>(call: () => IPromise<T>, success?: (result: T) => void, loading?: (loading: boolean) => void, fail?: (reason: any) => void): void {
        if (!ObjectExtensions.isNull(loading)) loading(true);

        call()
            .then(result => {
                if (!ObjectExtensions.isNull(loading)) loading(false);
            ...
            .catch(error => {
                ...
                if (!ObjectExtensions.isNull(fail)) fail(error);
            });
    }

    protected getOptions<T>(instanceAttributes: IAttributes, optionsParameter: string): T {
        if (StringExtensions.isNullOrWhiteSpace(instanceAttributes[optionsParameter])) return null;
    ...
    protected tryGetInt(options: any, instanceAttributes: IAttributes, optionFrom: string, optionTo?: string): number {
        ...
        if (!ObjectExtensions.isNull(instanceAttributes[optionFrom])) options[optionTo] = parseInt(instanceAttributes[optionFrom], 10);
    ...
    protected tryGetNumber(options: any, instanceAttributes: IAttributes, optionFrom: string, optionTo?: string): number {
        ...
        if (!ObjectExtensions.isNull(instanceAttributes[optionFrom])) options[optionTo] = parseFloat(instanceAttributes[optionFrom]);
    ...
    protected tryGetDate(options: any, instanceAttributes: IAttributes, optionFrom: string, optionTo?: string): number {
        ...
        if (!ObjectExtensions.isNull(instanceAttributes[optionFrom])) options[optionTo] = new Date(instanceAttributes[optionFrom]);
    ...
    protected tryGetBoolean(options: any, instanceAttributes: IAttributes, optionFrom: string, optionTo?: string): boolean {
        ...
        if (!ObjectExtensions.isNull(instanceAttributes[optionFrom])) {
            ...
            options[optionTo] = value === "yes" || value === "true" || value === "1";
        }
    ...
    protected tryGet(options: any, instanceAttributes: IAttributes, optionFrom: string, optionTo?: string): string {
        ...
        if (!ObjectExtensions.isNull(instanceAttributes[optionFrom])) options[optionTo] = instanceAttributes[optionFrom];
