...
export class CommentAreaDirective extends DirectiveBase {
    protected onInit(scope: IScope, instanceElement: JQuery, instanceAttributes: IAttributes, controller: IController, transclude: ITranscludeFunction): void {
        ...
        if (options.restrictEntry) {
            ...
            scope.$watch(
                () => scope["ngModel"],
                () => this.restrictEntry(null, options, instanceElement, false)
            );
        } else {
            instanceElement.on("keypress", () => this.showCharacterLeft(options, instanceElement, true));
            scope.$watch(
                () => scope["ngModel"],
                () => this.showCharacterLeft(options, instanceElement, false)
            );
        }
    }
        ...
        if (!ObjectExtensions.isNull(options.maxSize)) {
            ...
            if (!ObjectExtensions.isNull(options.maxSizeText)) {
                ...
                if (!ObjectExtensions.isNull(options.negativeClass)) {
                    if (available < 0) field.addClass(options.negativeClass);
                    else field.removeClass(options.negativeClass);
                }
    ...
    private checkSize(options: ICommentAreaParameters, element: JQuery): void {
        ...
        if (control.scrollHeight > control.clientHeight) {
            if (control.scrollHeight > defaultHeight) control.style.height = control.scrollHeight + "px";
            else control.style.height = defaultHeight + "px";
