...
export class KeyboardListenerDirective extends DirectiveBase {
    ...
    protected onInit(scope: IKeyboardListenerScope, instanceElement: JQuery, instanceAttributes: IAttributes, controller: IController, transclude: ITranscludeFunction): void {
        ...
        if (actions[actions.length - 1] === `|`) actions = actions.substr(0, actions.length - 1);
        ...
        if (keyActions.any(x => x.eventType === `keypress`)) attachTo.on(`keypress.${id}.documentKeyboard`, evaluateKeyPress);

        if (keyActions.any(x => x.eventType === `keydown`)) attachTo.on(`keydown.${id}.documentKeyboard`, evaluateKeyDown);

        if (keyActions.any(x => x.eventType === `keyup`)) attachTo.on(`keyup.${id}.documentKeyboard`, evaluateKeyUp);
    }
