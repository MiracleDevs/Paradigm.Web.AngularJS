...
export class OnKeyboardDirective extends DirectiveBase {
    ...
    protected onInit(scope: IScope, instanceElement: JQuery, instanceAttributes: IAttributes, controller: IController, transclude: ITranscludeFunction): void {
        ...
        if (keyActions.any(x => x.eventType === "keypress")) instanceElement.keypress(e => this.keyProcessor.evaluateKeyActions(keyActions, "keypress", scope, e as any));

        if (keyActions.any(x => x.eventType === "keydown")) instanceElement.keydown(e => this.keyProcessor.evaluateKeyActions(keyActions, "keydown", scope, e as any));

        if (keyActions.any(x => x.eventType === "keyup")) instanceElement.keyup(e => this.keyProcessor.evaluateKeyActions(keyActions, "keyup", scope, e as any));
    }
