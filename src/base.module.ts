...
export abstract class ModuleBase implements IModule {
    ...
    constructor() {
        ...
        this.register();
        this.module.config([
            AngularServices.stateProvider,
            AngularServices.urlRouterProvider,
            AngularServices.httpProvider,
            AngularServices.locationProvider,
            (stateProvider, urlRouterProvider, httpProvider, location) => this.configureRoutes(stateProvider, urlRouterProvider, httpProvider, location),
        ]);
    ...
    registerController(controller: Function): void {
        if (ObjectExtensions.isNull(controller) || ObjectExtensions.isNull(controller[registrableMetadataKey]))
        ...
        if (!ObjectExtensions.isNull(controllerInfo.dependencies)) controller.$inject = this.getDedependencies(controllerInfo.dependencies);

        this.module.controller(controllerInfo.name, controller as any);
        this.logger.debug(`registering ${controllerInfo.name}`);

        if (this.stateProvider == null) this.controllersQueue.push(controller);
        else this.registerControllerState(controller);
    }
    ...
    registerComponent(component: Function): void {
        if (ObjectExtensions.isNull(component) || ObjectExtensions.isNull(component[registrableMetadataKey]))
    ...
    registerService(service: Function): void {
        if (ObjectExtensions.isNull(service) || ObjectExtensions.isNull(service[registrableMetadataKey]))
        ...
        if (ObjectExtensions.isNull(serviceInfo.factory)) throw new Error("Missing service factory.");
    ...
    registerInterceptor(interceptor: Function): void {
        if (ObjectExtensions.isNull(interceptor) || ObjectExtensions.isNull(interceptor[registrableMetadataKey]))
        ...
        if (this.httpProvider == null) this.interceptorsQueue.push(interceptor);
        else {
    ...
    registerFilter(filter: Function): void {
        if (ObjectExtensions.isNull(filter) || ObjectExtensions.isNull(filter[registrableMetadataKey]))
    ...
    registerDirective(directive: Function): void {
        if (ObjectExtensions.isNull(directive) || ObjectExtensions.isNull(directive[registrableMetadataKey]))
    ...
    private registerControllerState(controller: Function): void {
        ...
        if (controllerInfo.stateName == null) return;
    ...
    private getDedependencies(dependencies: Array<string | Function>): string[] {
        if (ObjectExtensions.isNull(dependencies)) return null;

        return ArrayExtensions.select(dependencies, x =>
            ObjectExtensions.getTypeName(x) === "String" ? (x as string) : getRegistrableName(x as Function) || FunctionExtensions.getFunctionName(x as Function)
        );
    }
