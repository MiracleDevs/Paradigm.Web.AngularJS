...
import { DateExtensions, StringExtensions } from "@miracledevs/paradigm-ui-web-shared";
...
export class LoggingService extends ServiceBase {
    ...
    setCustomMessage(type: LogType, message: string): void {
        if (type < 0 || type >= this.messages.length) throw new Error(LoggingService.typeNotRecognized);
    ...
    log(message: string, type: LogType = LogType.Trace, tag: string = null): void {
        ...
        if (type < this.minimumLevel) return;
