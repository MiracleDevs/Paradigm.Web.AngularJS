...
export class KeyProcessorService extends ServiceBase {
    ...
    parseActions(text: string): ArrayList<KeyAction> {
        ...
        for (let i = 0; i < actionStrings.length; i++) {
            ...
            if (parts.length !== 2) throw new Error(`The key-submit expression number ${i} is incorrect. Should be event(keycode,[shift],[alt],[ctrl]): action`);

            const firstParenthesis = parts[0].indexOf("(");
            const lastParenthesis = parts[0].indexOf(")");

            if (firstParenthesis < 0 || lastParenthesis < 0) throw new Error(`The key-submit expression number ${i} is incorrect. Should be event(keycode,[shift],[alt],[ctrl]): action`);

            const eventType = parts[0].substr(0, firstParenthesis);

            if (eventType !== "keypress" && eventType !== "keydown" && eventType !== "keyup") throw new Error(`Only keypress, keydown and keyup events are allowed on action strings.`);
            ...
            if (parameters.any(x => x === "shift")) keyAction.shift = true;
            else if (parameters.any(x => x === "!shift")) keyAction.shift = false;

            if (parameters.any(x => x === "ctrl")) keyAction.ctrl = true;
            else if (parameters.any(x => x === "!ctrl")) keyAction.ctrl = false;

            if (parameters.any(x => x === "alt")) keyAction.alt = true;
            else if (parameters.any(x => x === "!alt")) keyAction.alt = false;
