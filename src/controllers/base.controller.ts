...
import { IScope, IPromise, translate } from "angular";
...
export abstract class ControllerBase extends AngularControllerBase {
    ...
    protected call<T>(call: () => IPromise<T>, success?: (result: T) => void, loading?: (loading: boolean) => void, fail?: (reason: any) => void): void {
        ...
        call()
            .then(result => {
                ...
                if (!ObjectExtensions.isNull(success)) success(result);
            })
            .catch(error => {
                if (!ObjectExtensions.isNull(loading)) loading(false);
    ...
    protected handleException(ex: any): void {
        if (ObjectExtensions.isNull(ex)) return;

        // if ex.data is not null, it's probably a
        // an http promise exception.
        if (!ObjectExtensions.isNull(ex.data)) ex = ex.data;
    ...
    protected unregisterMessage(token: RegistrationToken): void {
        if (!this.messageBusTokens.any(x => x.guid === token.guid)) throw new Error("Can not unregister a token registered outside this controller.");
