...
export function Service(parameters?: IService): <T>(constructor: { new (...args: any[]): T }) => void {
    ...
    return <T>(constructor: { new (...args: any[]): T }): void => {
        if (ObjectExtensions.isNull(constructor)) throw new Error("Missing service constructor.");

        if (ObjectExtensions.isNull(parameters.factory)) parameters.factory = constructor["factory"];

        if (ObjectExtensions.isNull(parameters.factory)) throw new Error("Missing service factory.");
